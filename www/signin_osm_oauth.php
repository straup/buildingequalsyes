<?php
	...
	if (! $GLOBALS['cfg']['crypto_oauth_cookie_secret']){
		...
		$GLOBALS['smarty']->display("page_signin_osm_oauth.txt");
		exit();
	}
	...
	$rsp = osm_oauth_get_request_token();
	...
	# part where we redirect the user to Osm to ask for
	...
	$url = osm_oauth_get_auth_url($args, $user_keys);
